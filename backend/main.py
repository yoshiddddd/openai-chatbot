...
# JSONファイルからサークル情報を読み込む関数
# def load_circle_info(file_path):
#     with open(file_path, 'r', encoding='utf-8') as jsonfile:
#         circle_data = json.load(jsonfile)  # JSONファイルを辞書として読み込む
#     return circle_data
...
def generate_response(circle_info, question):
    print(circle_info)
    prompt = f"ユーザーの質問: '{question}' に対してだけの返答を、以下のサークル情報を参考にして的確な情報をアドバイスする形で提供してください。余計な情報は含まないでください: '{circle_info}'"
...
#     # circle_data = load_circle_info('test.json')
#     print(circle_data)
#     # ユーザーからの質問
#     question = ""
...
#         # available_fields = list(circle_data.keys())

#         # 質問に最も関連するフィールド数を指定（質問内のフィールド数に応じてkを指定）
#         # k = len(available_fields)  # フィールドの数に基づいて動的に取得する
#         # k = 10
#         # k = determine_k(question, available_fields)+1
#         # k = 20
#         # print(k)
